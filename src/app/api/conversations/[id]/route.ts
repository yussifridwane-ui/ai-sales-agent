...
export async function GET(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const messages = findMany<Message>("messages", { conversationId: id }, { orderBy: "createdAt ASC" });
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    updateWhere("conversations", { id, organizationId: org.id }, body as Record<string, unknown>);
